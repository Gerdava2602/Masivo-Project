#Dash app that displays the table 'actividades' from the database and let the user download it.
...
app = DjangoDash(table_name, add_bootstrap_links=True, external_stylesheets=[dbc.themes.BOOTSTRAP])

table = variables.oferta.head(20)
...
def generate_csv(n_clicks, rows):
    ...
    new_table = variables.oferta.head(rows)
